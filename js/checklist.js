...
function renderizarLista(container, campos, comObservacao) {
  container.innerHTML = "";
  container.className = "lista-checklist grade-3col";
  ...
  grupos.forEach(function (grupo) {
    ...
    if (grupo.tipo === "par") {
      const obrigatorio = grupo.E.obrigatorio === "Sim" || grupo.D.obrigatorio === "Sim";
      item.dataset.par = "1";

      const nome = document.createElement("div");
      nome.className = "nome-item-checklist";
      nome.textContent = grupo.base + (obrigatorio ? " *" : "");
      ...
      [{ letra: "E", campo: grupo.E }, { letra: "D", campo: grupo.D }].forEach(function (lado) {
        ...
        select.dataset.obrigatorio = lado.campo.obrigatorio;
      ...
      if (comObservacao) {
        ...
        obs.type = "text";
        obs.className = "observacao-item-par";
      ...
      select.dataset.obrigatorio = campo.obrigatorio;
...
document.getElementById("form-checklist").addEventListener("submit", async function (evento) {
  ...
  todosOsSelects.forEach(function (select) {
    ...
    const item = select.closest(".item-checklist-grade");
    const obs = item ? item.querySelector(".observacao-item, .observacao-item-par") : null;

    if (select.dataset.obrigatorio === "Sim" && !valor) {
      faltouObrigatorio = true;
      select.style.borderColor = "var(--erro)";
    }
    if (valor) {
      respostas.push({ id_campo: select.dataset.idCampo, resposta: valor, observacao: obs ? obs.value : "" });
    }
  });
