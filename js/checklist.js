...
function renderizarLista(container, campos, comObservacao) {
  ...
  campos.forEach(function (campo) {
    ...
    linha.className = comObservacao ? "linha-item-checklist" : "linha-item-checklist acessorio";
    linha.dataset.idCampo = campo.id;
    linha.dataset.obrigatorio = campo.obrigatorio;

    const nome = document.createElement("div");
    nome.className = "nome-item-checklist";
    nome.textContent = campo.nome + (campo.obrigatorio === "Sim" ? " *" : "");
    linha.appendChild(nome);

    const select = document.createElement("select");
    select.className = "resposta-item";
    const opcoes = comObservacao ? ["", "Ok", "Não Ok", "Não Checado"] : ["", "Sim", "Não"];
    opcoes.forEach(function (opcao) {
      const el = document.createElement("option");
      el.value = opcao;
      el.textContent = opcao === "" ? "Selecione" : opcao;
...
document.getElementById("form-checklist").addEventListener("submit", async function (evento) {
  ...
  if (!documento || !nome || !placa) {
    mensagem.textContent = "Preencha o documento, o nome e a placa antes de salvar.";
    mensagem.classList.add("erro");
    return;
  }

  const linhasResposta = document.querySelectorAll(".linha-item-checklist");
  const respostas = [];
  let faltouObrigatorio = false;

  linhasResposta.forEach(function (linha) {
    const select = linha.querySelector(".resposta-item");
    const obs = linha.querySelector(".observacao-item");
    const valor = select ? select.value : "";

    if (linha.dataset.obrigatorio === "Sim" && !valor) {
      faltouObrigatorio = true;
      select.style.borderColor = "var(--erro)";
    }
    if (valor) {
      respostas.push({ id_campo: linha.dataset.idCampo, resposta: valor, observacao: obs ? obs.value : "" });
    }
  });

  if (faltouObrigatorio) {
    mensagem.textContent = "Preencha todos os itens marcados com * antes de salvar.";
    mensagem.classList.add("erro");
    return;
  }
