...
let fotosExtras = []; // { nomeArquivo, tipoMime, base64 } — opcionais, além dos 4 lados

// Os 4 lados obrigatórios do veículo. "slug" é usado nos ids dos
// elementos (input-foto-<slug>, quadro-foto-<slug>) e "nome" é o que
// vai salvo na aba Fotos_Videos (coluna Tipo) e mostrado na tela.
const LADOS_FOTOS_OBRIGATORIAS = [
  { slug: "frente", nome: "Frente" },
  { slug: "traseira", nome: "Traseira" },
  { slug: "lateral-esquerda", nome: "Lateral Esquerda" },
  { slug: "lateral-direita", nome: "Lateral Direita" },
];
const fotosLados = {}; // { "Frente": { nomeArquivo, tipoMime, base64 }, ... }
...
if (usuario) {
  ...
  if (!idModelo) {
    document.getElementById("mensagem-salvar").textContent = "Nenhum modelo de check list selecionado. Volte pra Novo Check List.";
...
// Agrupa a lista de campos em: pares (E+D juntos) e itens soltos —
// preservando a posição de cada um na ordem original (a coluna
// Ordem, vinda do backend). Um par entra na posição em que a
// primeira metade dele apareceu, não no final da lista.
function agruparPares(campos) {
  const indicePorBase = {}; // base -> posição em grupos
  const grupos = [];
  ...
  campos.forEach(function (campo) {
    ...
    grupos[indicePorBase[info.base]][info.lado] = campo;
  });

  // Se algum "par" só achou um dos dois lados, vira solto — na mesma
  // posição em que estava, sem pular pro final.
  return grupos.map(function (grupo) {
    if (grupo.tipo === "par" && (!grupo.E || !grupo.D)) {
      return { tipo: "solto", campo: grupo.E || grupo.D };
    }
    return grupo;
  });
}
...
// Diz pro navegador quantas linhas cabem em cada coluna antes de
// pular pra próxima — sem isso, "grid-auto-flow: column" não sabe
// onde quebrar e põe tudo numa coluna só. Recalcula com base na
// largura da tela agora (os mesmos números dos breakpoints do CSS).
function aplicarLeituraColunar(container, quantidadeItens) {
  const largura = window.innerWidth;
  let colunas;
  if (container.classList.contains("grade-7col")) {
    colunas = largura <= 720 ? 1 : 7;
  } else {
    colunas = largura <= 720 ? 1 : largura <= 1100 ? 3 : 5;
  }
  const linhas = Math.max(Math.ceil(quantidadeItens / colunas), 1);
  container.style.gridTemplateRows = "repeat(" + linhas + ", auto)";
}

// Máscara de data: a pessoa digita só os números (ddmmaaaa) e o
// campo insere as barras sozinho, conforme ela digita.
...
function renderizarLista(container, campos, comObservacao) {
  ...
  // Dois campos especiais da etiqueta de troca de óleo — não são
  // item Ok/Não Ok, são pra digitar (Km e Data). Só fazem sentido na
  // seção Técnica, logo antes de "Nível de óleo do motor".
  if (comObservacao) {
    const indiceOleo = grupos.findIndex(function (g) {
      return g.tipo === "solto" && g.campo.nome === "Nível de óleo do motor";
    });
    const posicao = indiceOleo === -1 ? 0 : indiceOleo;
    grupos.splice(posicao, 0, { tipo: "km-etiqueta" }, { tipo: "data-etiqueta" });
  }
  ...
  grupos.forEach(function (grupo) {
    ...
    if (grupo.tipo === "km-etiqueta") {
      const nome = document.createElement("div");
      nome.className = "nome-item-checklist";
      nome.textContent = "Km última troca de óleo";
      item.appendChild(nome);

      const input = document.createElement("input");
      input.type = "text";
      input.id = "km-ultima-troca-oleo";
      input.placeholder = "Ex: 32000";
    ...
    if (grupo.tipo === "data-etiqueta") {
      const nome = document.createElement("div");
      nome.className = "nome-item-checklist";
      nome.textContent = "Data última troca de óleo";
      item.appendChild(nome);

      const input = document.createElement("input");
      input.type = "text";
      input.id = "data-ultima-troca-oleo";
  ...
  aplicarLeituraColunar(container, grupos.length);
}
...
// ---------------------------------------------------------------
// Solicitações do Cliente — começa com uma linha, o botão adiciona
// mais conforme o cliente for pedindo coisas.
// ---------------------------------------------------------------

function adicionarLinhaSolicitacao() {
  const linha = document.createElement("input");
  linha.type = "text";
  linha.className = "linha-solicitacao";
  linha.placeholder = "Ex: Cliente relatou barulho ao frear";
  document.getElementById("lista-solicitacoes").appendChild(linha);
}

adicionarLinhaSolicitacao();

document.getElementById("botao-add-solicitacao").addEventListener("click", adicionarLinhaSolicitacao);
...
// "Salvar em PDF" abre o diálogo de impressão do navegador — quem
// escolhe "Salvar como PDF" (ou "Microsoft Print to PDF") consegue
// escolher a pasta de destino ali mesmo, sem precisar de nada extra
// no sistema. O CSS de impressão (estilo.css) esconde os botões e
// campos de busca, deixando só o conteúdo do check list e as fotos.
document.getElementById("botao-pdf-checklist").addEventListener("click", function () {
  window.print();
});
...
// Os 4 lados obrigatórios: cada quadro abre o seletor de arquivo ao
// ser clicado, mostra a miniatura no lugar do texto e ganha um "x"
// pra poder trocar a foto antes de salvar.
LADOS_FOTOS_OBRIGATORIAS.forEach(function (lado) {
  const input = document.getElementById("input-foto-" + lado.slug);
  const quadro = document.getElementById("quadro-foto-" + lado.slug);
  ...
  input.addEventListener("change", function (evento) {
    ...
    leitor.onload = function () {
      ...
      fotosLados[lado.nome] = { nomeArquivo: arquivo.name, tipoMime: arquivo.type || "image/jpeg", base64: base64 };

      quadro.classList.add("preenchido");
      quadro.classList.remove("faltando");
      quadro.innerHTML = '<img src="' + resultado + '" alt="Foto ' + lado.nome + '" />' +
        '<button type="button" class="botao-remover-foto-lado" title="Remover">×</button>';

      quadro.querySelector(".botao-remover-foto-lado").addEventListener("click", function (evClique) {
        evClique.stopPropagation();
        delete fotosLados[lado.nome];
        quadro.classList.remove("preenchido");
        quadro.innerHTML = '<span class="texto-quadro-foto">Toque para adicionar</span>';
      });
    };
...
// Fotos extras (opcionais) — mesma lógica de antes, sem limite.
document.getElementById("input-fotos-extra").addEventListener("change", function (evento) {
  ...
  arquivos.forEach(function (arquivo) {
    ...
    leitor.onload = function () {
      const resultado = leitor.result;
      const base64 = resultado.split(",")[1];
      fotosExtras.push({ nomeArquivo: arquivo.name, tipoMime: arquivo.type || "image/jpeg", base64: base64 });
      const area = document.getElementById("area-fotos-extra");
...
document.getElementById("form-checklist").addEventListener("submit", async function (evento) {
  ...
  // As 4 fotos do veículo (Frente, Traseira, Lateral Esquerda, Lateral
  // Direita) são obrigatórias — sem elas, nem chega a criar a OS.
  const ladosFaltando = [];
  LADOS_FOTOS_OBRIGATORIAS.forEach(function (lado) {
    const quadro = document.getElementById("quadro-foto-" + lado.slug);
    if (fotosLados[lado.nome]) {
      quadro.classList.remove("faltando");
    } else {
      ladosFaltando.push(lado.nome);
      quadro.classList.add("faltando");
    }
  });
  if (ladosFaltando.length > 0) {
    mensagem.textContent = "Faltam fotos obrigatórias: " + ladosFaltando.join(", ") + ".";
    mensagem.classList.add("erro");
    document.getElementById("mensagem-fotos").textContent = "Envie as 4 fotos do veículo antes de salvar.";
    document.getElementById("mensagem-fotos").className = "mensagem-inline erro";
    return;
  }
  document.getElementById("mensagem-fotos").textContent = "";
  document.getElementById("mensagem-fotos").className = "mensagem-inline";
  ...
  const kmUltimaTrocaOleo = document.getElementById("km-ultima-troca-oleo") ? document.getElementById("km-ultima-troca-oleo").value : "";
  const dataUltimaTrocaOleo = document.getElementById("data-ultima-troca-oleo") ? document.getElementById("data-ultima-troca-oleo").value : "";
  const solicitacoesCliente = Array.from(document.querySelectorAll(".linha-solicitacao"))
    .map(function (linha) { return linha.value.trim(); })
  ...
  try {
    ...
    const respostaChecklist = await chamarApi({
      ...
      solicitacoes_cliente: solicitacoesCliente,
    });
    ...
    const fotosParaEnviar = LADOS_FOTOS_OBRIGATORIAS
      .map(function (lado) { return { foto: fotosLados[lado.nome], tipo: lado.nome }; })
      .concat(fotosExtras.map(function (foto) { return { foto: foto, tipo: "Foto" }; }));

    for (let i = 0; i < fotosParaEnviar.length; i++) {
      botao.textContent = "Enviando fotos (" + (i + 1) + "/" + fotosParaEnviar.length + ")...";
      await enviarFoto(idOS, fotosParaEnviar[i].foto, fotosParaEnviar[i].tipo);
    }

    mensagem.textContent = "Check List salvo. OS " + idOS + " criada e a Parte 1 já está travada pra edição.";
    mensagem.classList.add("sucesso");
    botao.textContent = "Salvo";
    document.getElementById("botao-pdf-checklist").classList.remove("oculto");
  } catch (erro) {
...
async function enviarFoto(idOS, foto, tipo) {
  try {
    await chamarApi({
      acao: "salvarFotoOS",
      id_os: idOS,
