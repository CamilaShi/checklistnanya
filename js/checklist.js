...
function renderizarLista(container, campos, comObservacao) {
  ...
  // Campo especial "Km" (última troca de óleo, da etiqueta do
  // veículo) — não é um item Ok/Não Ok, é um número pra digitar. Só
  // faz sentido na seção Técnica, logo antes de "Nível de óleo do
  // motor" (entra ali mesmo se esse item não existir, por segurança,
  // vai só no início da lista).
  if (comObservacao) {
    const indiceOleo = grupos.findIndex(function (g) {
      return g.tipo === "solto" && g.campo.nome === "Nível de óleo do motor";
    });
    grupos.splice(indiceOleo === -1 ? 0 : indiceOleo, 0, { tipo: "km-etiqueta" });
  }
